    ...
    <div className="max-w-4xl mx-auto px-4 py-8 space-y-6">
      ...
      <div className="bg-white border border-slate-200/80 rounded-2xl p-5 shadow-sm space-y-4">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          ...
          <button
            ...
            className={`px-4.5 py-2.5 rounded-xl text-xs font-semibold flex items-center gap-2 transition-all cursor-pointer border ${isStarredOnly
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-1">
          ...
          <div className="relative">
            ...
            <select
              ...
              className="w-full text-xs p-3 pl-9 pr-8 bg-slate-50 border border-slate-200 outline-none rounded-xl transition-colors text-slate-700 cursor-pointer appearance-none"
            >
      ...
      {!isStarredOnly && (
        <div className="grid grid-cols-3 gap-2 text-center bg-slate-100/80 p-1.5 rounded-2xl">
          {[
            { dayNum: 1, label: 'Day 1: July 2', subtitle: 'Thursday' },
            { dayNum: 2, label: 'Day 2: July 3', subtitle: 'Friday' },
            { dayNum: 3, label: 'Day 3: July 4', subtitle: 'Saturday' }
          ].map((tab) => (
            <button
              key={tab.dayNum}
              onClick={() => {
              ...
              className={`p-3 rounded-xl transition-all cursor-pointer ${selectedDay === tab.dayNum
      ...
      <div className="space-y-4">
        {filteredSessions.length === 0 ? (
          ...
          <>
            {selectedDay === 1 && !isStarredOnly && (
              <div className="bg-[#000a1e] text-white rounded-2xl p-5 flex items-start justify-between gap-4 border border-[#fed65b]/20 mb-1 shadow-sm">
                <div className="flex items-start gap-4 flex-1">
                  <div className="p-2.5 bg-[#fed65b]/15 text-[#ffe088] rounded-xl shrink-0 mt-1">
                    <Info className="w-5 h-5" />
                  </div>
                  <div className="space-y-1">
                    <h4 className="font-serif text-sm font-bold text-[#ffe088]">Parking</h4>
                    <p className="text-xs text-slate-300 leading-relaxed">
                      Parking is in the <strong className="text-white"><a href="https://maps.app.goo.gl/hUS8n5g1CueM25Bv9" target="_blank" rel="noreferrer" className="text-white hover:text-[#ffe088] underline underline-offset-2 inline-flex items-center gap-1 transition-colors">Student Residence Parking <ExternalLink className="w-3 h-3" /></a></strong> adjacent to the RCC building.</p>
                  </div>
                </div>
                <div
                  className="w-16 h-16 sm:w-20 sm:h-20 shrink-0 rounded-xl overflow-hidden border border-[#fed65b]/20 cursor-pointer hover:opacity-80 transition-opacity flex items-center justify-center bg-white/5"
                  onClick={() => setIsLightboxOpen(true)}
                  title="View RCC Front Entrance"
                >
                  <img src={frontEntrance} alt="RCC Front Entrance" className="w-full h-full object-cover opacity-90" />
                </div>
              </div>
            )}
            ...
            {filteredSessions.map((session) => {
              ...
              return (
                <div
                  key={session.id}
                  className={`bg-white border rounded-2xl transition-all shadow-xs overflow-hidden ${isExpanded ? 'border-[#fed65b] ring-1 ring-[#fed65b]/40 shadow-sm' : 'border-slate-200/70 hover:border-slate-300/80 bg-[#fbfbfc]'
                    }`}
                >
                  {/* Visual Header Block */}
                  <div
                  ...
                  >
                    <div className="space-y-2.5 min-w-0 flex-1">
                      ...
                      <div className="flex flex-wrap items-center gap-2">
                        <span
                          onClick={(e) => { e.stopPropagation(); setSelectedCategory(session.category); }}
                          className={`text-[10px] uppercase font-bold tracking-wider px-2 py-0.5 rounded-full border cursor-pointer hover:opacity-80 transition-opacity ${getCategoryColor(session.category)}`}
                          title={`Filter by ${session.category}`}
                        >
                          {session.category}
                        </span>
                        {isStarredOnly && (
                    ...
                    <div className="flex items-center gap-2 mt-1 shrink-0" onClick={(e) => e.stopPropagation()}>
                      ...
                      <button
                        ...
                        className={`p-2 rounded-xl transition-all cursor-pointer ${isStarred
      ...
      {isLightboxOpen && (
        ...
        >
          ...
          <div className="flex flex-col items-center justify-center max-w-full h-full p-2 sm:p-4" onClick={(e) => e.stopPropagation()}>
            <img
              src={frontEntrance}
              alt="RCC building"
